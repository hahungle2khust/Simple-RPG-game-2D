...
namespace Test
{
    static class SkillManager
    {
        ...
        public static List<BaseSkill> CooldownSkills = new List<BaseSkill>();
        ...
        public static void Update()
        {
            if(Game.World != null)
            {
                ...
                for(int i=0; i<CooldownSkills.Count; i += 1)
                {
                    CooldownSkills[i].Cooldown -= Game.ElapsedGameTime;
                    if(CooldownSkills[i].Cooldown <= 0)
                    {
                        CooldownSkills[i].Cooldown = 0;
